...
using CSV, MixedModels, DataFrames
include("zscore.jl")
...
surp_col     = find(names(surprisal).==surp_col_name)[1]            # Find column index of first network's surprisal value
prevsurp_col = length(surprisal)+1                                  # Columns for previous-word surprisal will start here

for c = surp_col:length(surprisal)
